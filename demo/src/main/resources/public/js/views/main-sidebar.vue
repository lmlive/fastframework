...
<script>
    ...
    define(['vue','validate', 'v!views/main-sidebar-sub-menu'], function (Vue,  validate) {
        ...
        return Vue.component('main-sidebar',{
            ...
            methods: {
                ...
                routeHandle(route) {
                    if (route.meta.isTab) {
                        // tab选中, 不存在先添加
                        var tab = this.mainTabs.filter(function(item){ return item.name === route.name})[0]
                        if (!tab) {
                            if (route.meta.isDynamic) {
                                route = this.dynamicMenuRoutes.filter(function(item) {return item.name === route.name})[
                                    0]
                                if (!route) {
